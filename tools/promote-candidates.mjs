...
// 2026-09-23設計: Googleシートは読まない（ローカル+アプリを正本にする運用）。
// GASが落ちていても（候補ステータスが空でも）昇格なしで処理を続行し、無人実行を止めない。
...
let statusByName = {};
let statusFetchNote = '取得できず（今回は昇格なしで続行）';
try {
  ...
  if (res.ok) {
    ...
    if (!body.error) {
      statusByName = body.candidateStatuses || {};
      statusFetchNote = `OK（${Object.keys(statusByName).length}件）`;
    } else {
      statusFetchNote = `GAS error: ${body.error}`;
    }
  } else {
    statusFetchNote = `HTTP ${res.status}`;
  }
} catch (err) {
  statusFetchNote = `例外: ${err.message}`;
}
...
（詳細は PROJECT_NOTES.md セクションC）。候補ステータス取得: ${statusFetchNote}
...
console.log(`OK: ${outPath} を作成（名簿${newPerformers.length}名／昇格${promotable.length}件／候補ステータス取得: ${statusFetchNote}）`);
