...
お気に入り・コメント機能・候補の選考ステータスのバックエンド（Google Apps Script）をデプロイする手順。
Googleアカウントでの操作が必要（この手順はAIエージェントが自動実行できない）。

> **2026-09-23更新**: 候補パイプライン機能（☆お気に入り・💬コメントを候補カードにも追加／
> 選考ステータス「未確認・検討中・連絡済み・採用・保留・見送り」の追加）に伴い `gas/Code.gs` を更新した。
> 併せて「Candidates」タブへの自動追記（addCandidates）は廃止し、候補一覧の正本は
> `scouting-report/candidates/`（ローカル）に一本化した。**この変更を反映するには、
> 下記「6. コードを更新したいとき」の手順で再デプロイが必要。** 再デプロイまでは候補カードの
> ステータス変更・☆・コメントがエラーになる（候補の閲覧自体はできる）。
...
## 2. スクリプトプロパティを設定する（コードには書かない）

1. 左メニューの歯車アイコン「プロジェクトの設定」を開く
2. 「スクリプト プロパティ」→「スクリプト プロパティを追加」
3. 次の2つを追加して保存:
   - `PASSPHRASE`　値: アプリの合言葉（`.secret` と同じ文字列）
   - `SHEET_ID`　値: 対象スプレッドシートのID（URLの `/d/` と `/edit` の間の文字列）

※コードは `getActiveSpreadsheet()` ではなく `openById(SHEET_ID)` でシートを開く。
`getActiveSpreadsheet` の `spreadsheets.currentonly` 権限は、他の権限（メール送信等）を
追加した際に壊れて匿名Webアプリ全体が動かなくなる不具合があるため、標準の
`spreadsheets` 権限を使う `openById` に統一している（2026-07-15の障害対応）。
初回は `authorizeApp` 関数を一度実行して `spreadsheets` 権限を承認すること。
